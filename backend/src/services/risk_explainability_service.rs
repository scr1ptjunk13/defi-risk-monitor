        ...
        if risk_metrics.liquidity_score.to_string().parse::<f64>().unwrap_or(0.0) > 0.4 {
            let liquidity_factor = RiskFactor {
                ...
                explanation: format!(
                    ...
                    _pool_state.tvl_usd.as_ref().map(|v| v.to_string()).unwrap_or_else(|| "N/A".to_string())
                ),
                current_value: _pool_state.tvl_usd.clone(),
    ...
    async fn generate_position_context(
        ...
        explanation.position_context = PositionContext {
            ...
            time_context: TimeContext {
                position_age_hours: position.created_at.map(|dt| (chrono::Utc::now() - dt).num_hours() as i32).unwrap_or(0),
